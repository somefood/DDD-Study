        ...
        if (orderRequest == null) {
            ...
            if (orderRequest.getShippingInfo() == null) {
                ...
                if (orderRequest.getShippingInfo().getReceiver() == null) {
                    ...
                    if (!StringUtils.hasText(orderRequest.getShippingInfo().getReceiver().name())) {
                        errors.add(ValidationError.of("shippingInfo.receiver.name", "required"));
                    }
                    if (!StringUtils.hasText(orderRequest.getShippingInfo().getReceiver().phone())) {
                    ...
                    if (orderRequest.getShippingInfo().getAddress() == null) {
                        ...
                        if (!StringUtils.hasText(orderRequest.getShippingInfo().getAddress().zipCode())) {
                            errors.add(ValidationError.of("shippingInfo.address.zipCode", "required"));
                        }
                        if (!StringUtils.hasText(orderRequest.getShippingInfo().getAddress().address1())) {
